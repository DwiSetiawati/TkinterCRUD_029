...
def fetch_all():
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute('SELECT id, nama_siswa, bilogi, fisika, inggris, prediksi_fakultas FROM nilai_siswa ORDER BY id DESC')
    rows = cur.fetchall()
    conn.close()
    return rows
...
class NilaiApp:
    def __init__(self, root):
        ...
        try:
            ...
            pass
        style.configure('TLable', font=('Segoe UI', 10))
        ...
        frm_right = ttk.LabelFrame(root, text='DData Tersimpan', padding=(8,8))
